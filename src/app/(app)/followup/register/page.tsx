...
import { User, Phone, Mail, MapPin, Church, Heart, ShieldCheck } from 'lucide-react'
...
export default function RegisterBelieverPage() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(false)
    alert('Believer registered successfully!')
  }
                  ...
                  <div
                    className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all duration-300 ${
                      s < step
                        ...
                        : s === step
                        ? 'border-[#1F5E4A] text-[#1F5E4A] bg-white'
                        : 'border-gray-300 text-gray-400 bg-white'
                    }`}
                  >
                    {s < step ? '✓' : s}
                  </div>
                  <span className={`text-xs font-medium hidden sm:block ${s === step ? 'text-[#1F5E4A]' : 'text-gray-400'}`}>
                    {label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
              ...
              {step === 1 && (
                ...
                >
                  ...
                  <Button
                    ...
                    className="w-full h-11 bg-[#1F5E4A] hover:bg-[#174d3c] text-white font-semibold rounded-xl mt-2"
                  >
                    Continue
                  </Button>
                </motion.div>
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <div>
                    ...
                    <Input
                      type="date"
                      value={form.date_of_conversion}
                      onChange={e => set('date_of_conversion', e.target.value)}
                      className="h-11"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Textarea
                      placeholder="Describe how they gave their life to Christ..."
                      value={form.how_came_to_christ}
                      onChange={e => set('how_came_to_christ', e.target.value)}
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <div>
                    ...
                    <Textarea
                      placeholder="What would they like prayer for? Family, health, work, addictions..."
                      value={form.prayer_needs}
                      onChange={e => set('prayer_needs', e.target.value)}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full h-11 border border-gray-200 rounded-md text-sm bg-white px-3 focus:outline-none focus:border-[#1F5E4A]"
                    >
                      {['English', 'Yoruba', 'Igbo', 'Hausa', 'French', 'Swahili', 'Portuguese'].map(l => (
                  ...
                  <div
                    ...
                    className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${
                      form.consent ? 'border-[#1F5E4A] bg-[#f0f7f4]' : 'border-gray-200'
                    }`}
                  >
                    <div className={`w-5 h-5 rounded border-2 mt-0.5 flex items-center justify-center flex-shrink-0 ${
                      form.consent ? 'border-[#1F5E4A] bg-[#1F5E4A]' : 'border-gray-300'
                    }`}>
                      {form.consent && <span className="text-white text-xs">✓</span>}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-800 flex items-center gap-1">
                        <ShieldCheck className="w-4 h-4 text-[#1F5E4A]" /> Consent given
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        This person has consented to being contacted and followed up by our volunteers for discipleship purposes.
                      </p>
                    </div>
                  </div>
