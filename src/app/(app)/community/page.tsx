...
import { Plus, X, Heart, MessageCircle, Loader2, AlertCircle } from "lucide-react"
...
export default function CommunityPage() {
  ...
  async function loadPosts() {
    ...
    if (data) {
      setPosts(data.map((p: Post & { profiles?: { full_name: string } }) => ({
        ...p,
  ...
  const handlePost = async () => {
    ...
    const newPost = { ...data, author_name: data.profiles?.full_name ?? 'You' }
          ...
          >
            ...
            >
              ...
              <div className="flex gap-3">
                <Button variant="secondary" size="sm" onClick={() => setShowModal(false)} className="flex-1">Cancel</Button>
                <Button
                  size="sm"
                  onClick={handlePost}
